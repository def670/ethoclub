...
class Analyzer extends broccoli_plugin_1.default {
    constructor(inputTree, pack, treeType) {
        ...
        this.erasedImportKinds = new Set(['type', 'typeof']);
        // TypeScript: `export type foo from 'foo'`
        // Flow: doesn't have type-only exports
        this.erasedExportKinds = new Set(['type']);
    }
    setupParser() {
    ...
    parseImports(relativePath, source) {
        ...
        traverse_1.default(ast, {
            ...
            ImportDeclaration: path => {
                if (this.erasedImportKinds.has(path.node.importKind))
                    return;
                imports.push({
                    isDynamic: false,
            ...
            ExportNamedDeclaration: path => {
                if (!path.node.source)
                    return;
                if (this.erasedExportKinds.has(path.node.exportKind))
                    return;
                imports.push({
                    isDynamic: false,
                    specifier: path.node.source.value,
